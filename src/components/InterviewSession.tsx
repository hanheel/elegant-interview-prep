...
export function InterviewSession({
	...
	const generateFeedback = (score: number, style: "friend" | "interviewer") => {
		...
		const feedbacks =
			style === "friend" ? friendlyFeedbacks : interviewerFeedbacks;
		const category = score >= 90 ? "high" : "medium";
		return feedbacks[category][
				...
				<div className="container max-w-4xl mx-auto py-4">
					<div className="flex justify-between items-center mb-2">
						...
						<div className="flex items-center gap-4">
							...
							<Badge
								variant={settings.mode === "voice" ? "default" : "secondary"}
							>
						...
						<div className="border-t p-4">
							<div className="flex gap-2">
								<Input
									...
									onKeyPress={(e) =>
										e.key === "Enter" && !isAiSpeaking && handleSendMessage()
									}
									className="flex-1"
			...
			<Dialog open={showVoiceModal} onOpenChange={() => {}}>
				<DialogContent className="text-center">
					...
					<div className="py-8">
						...
						<Progress
							value={
								((settings.maxSpeakingTime ?? 0 - voiceTimeLeft) /
									(settings.maxSpeakingTime ?? 1)) *
								100
							}
							className="mt-4"
						/>
					</div>
					<Button onClick={handleVoiceComplete}>답변 완료</Button>
				</DialogContent>
			</Dialog>
		</div>
	);
